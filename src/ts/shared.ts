...
import { VenuuEvent, Filters } from "./types.js";
import { Utils } from "./utils.js";

class VenuuApp {
  private events: VenuuEvent[] = [];
  private filteredEvents: VenuuEvent[] = [];
  ...
  populateSelect(selectId: string, options: string[]): void {
  ...
  setupEventListeners(): void {
    ...
    if (locationFilter) {
      locationFilter.addEventListener("change", (e) => {
        this.currentFilters.location = (e.target as HTMLSelectElement).value;
        this.applyFilters();
      });
    }
  ...
  createEventElement(event: VenuuEvent): HTMLElement {
    ...
    const image =
      event.image || event.event_image || event.event_thumbnail || "";
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  (window as any).venuuApp = new VenuuApp();
});

// Make VenuuApp available globally
declare global {
  interface Window {
    venuuApp: VenuuApp;
  }
